...
<div class="space-y-4 sm:space-y-6">
    ...
    {{-- ====== FILTER BAR ====== --}}
    <div class="bg-white rounded-xl border border-slate-200 p-3 sm:p-4 no-print shadow-sm">
        <form action="{{ route('monitoring.index') }}" method="GET">
            {{-- Row 1: selects + filter button --}}
            <div class="flex flex-wrap items-end gap-3">
                <div class="flex-1 min-w-[120px]">
                    <label class="text-xs font-bold text-slate-400 uppercase tracking-wider block mb-1">{{ __('Bulan') }}</label>
                    <select name="bulan"
                            class="w-full rounded-lg border border-slate-300 bg-slate-50 text-slate-700 text-sm py-2 px-3 focus:border-blue-600 focus:ring-blue-600 focus:outline-none">
                ...
                <button type="submit"
                        class="bg-blue-600 hover:bg-blue-700 text-white font-bold px-4 py-2 rounded-lg transition text-sm flex items-center shadow-sm whitespace-nowrap">
            ...
            <div class="flex flex-wrap gap-2 mt-3">
                ...
                <button type="button" onclick="window.print()"
                        class="flex-1 sm:flex-none bg-slate-750 hover:bg-slate-850 text-white font-semibold px-4 py-2 rounded-lg transition inline-flex items-center justify-center text-sm shadow-sm whitespace-nowrap">
    ...
    <div class="grid grid-cols-2 lg:grid-cols-4 gap-3 sm:gap-4">
        {{-- Card 1 --}}
        <div class="bg-white rounded-xl border border-slate-200 border-l-4 border-l-blue-600 p-4 sm:p-5 flex items-center justify-between shadow-sm">
            <div>
                <p class="text-[10px] sm:text-xs font-bold text-slate-400 uppercase tracking-wider leading-tight">{{ __('Total Aset') }}</p>
                <p class="text-xl sm:text-2xl font-bold text-slate-800 mt-0.5">{{ $stats->total_aset ?? 0 }}</p>
            </div>
            <div class="w-9 h-9 sm:w-10 sm:h-10 rounded-lg bg-blue-50 flex items-center justify-center text-blue-600 flex-shrink-0">
                <i class="fas fa-tools text-base sm:text-lg"></i>
            </div>
        </div>
        {{-- Card 2 --}}
        <div class="bg-white rounded-xl border border-slate-200 border-l-4 border-l-emerald-500 p-4 sm:p-5 flex items-center justify-between shadow-sm">
            <div>
                <p class="text-[10px] sm:text-xs font-bold text-slate-400 uppercase tracking-wider leading-tight">{{ __('Total Waktu Kerja') }}</p>
                <p class="text-xl sm:text-2xl font-bold text-slate-800 mt-0.5">
                    {{ number_format($stats->total_waktu_kerja ?? 0, 1) }}
                    <span class="text-xs font-normal text-slate-500">{{ app()->getLocale() == 'en' ? 'hrs' : 'jam' }}</span>
                </p>
            </div>
        ...
        {{-- Card 3 --}}
        <div class="bg-white rounded-xl border border-slate-200 border-l-4 border-l-amber-500 p-4 sm:p-5 flex items-center justify-between shadow-sm">
            <div>
                <p class="text-[10px] sm:text-xs font-bold text-slate-400 uppercase tracking-wider leading-tight">{{ __('Rata-rata Idle') }}</p>
                <p class="text-xl sm:text-2xl font-bold text-slate-800 mt-0.5">{{ number_format($stats->avg_idle ?? 0, 1) }}%</p>
            </div>
        ...
        {{-- Card 4 --}}
        <div class="bg-white rounded-xl border border-slate-200 border-l-4 border-l-rose-500 p-4 sm:p-5 flex items-center justify-between shadow-sm">
            <div>
                <p class="text-[10px] sm:text-xs font-bold text-slate-400 uppercase tracking-wider leading-tight">{{ __('Total Bahan Bakar') }}</p>
                <p class="text-xl sm:text-2xl font-bold text-slate-800 mt-0.5">
    ...
    <div class="bg-white rounded-xl border border-slate-200 p-3 sm:p-6 shadow-sm">
        <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 mb-4">
            <h3 class="text-xs font-bold text-slate-500 uppercase tracking-wider flex items-center">
                ...
                {{ __('Ringkasan Aset') }}
                <span class="text-[10px] font-normal text-slate-400 normal-case ml-2">({{ __($bulan) }} {{ $tahun }})</span>
            </h3>
            <div class="relative w-full sm:max-w-xs">
                ...
                <input type="text" id="assetSearchInput" placeholder="{{ __('Cari Aset...') }}"
                       class="pl-8 pr-4 py-2 w-full border border-slate-300 rounded-lg text-sm bg-slate-50 text-slate-800 placeholder-slate-400 focus:ring-blue-600 focus:border-blue-600 focus:outline-none">
            </div>
        </div>
            ...
            <table class="min-w-full divide-y divide-slate-200 border border-slate-100 rounded-lg overflow-hidden">
                <thead class="bg-slate-50">
                    <tr>
                        ...
                        <th class="px-3 sm:px-4 py-3 text-left text-[10px] sm:text-xs font-bold text-slate-500 uppercase tracking-wider whitespace-nowrap hidden sm:table-cell">{{ __('Operasi') }}</th>
                        <th class="px-3 sm:px-4 py-3 text-left text-[10px] sm:text-xs font-bold text-slate-500 uppercase tracking-wider whitespace-nowrap hidden sm:table-cell">{{ __('Idle') }}</th>
                        <th class="px-3 sm:px-4 py-3 text-left text-[10px] sm:text-xs font-bold text-slate-500 uppercase tracking-wider whitespace-nowrap">{{ __('% Idle') }}</th>
                        <th class="px-3 sm:px-4 py-3 text-left text-[10px] sm:text-xs font-bold text-slate-500 tracking-wider whitespace-nowrap hidden md:table-cell">{{ __('BBM (L)') }}</th>
                        <th class="px-3 sm:px-4 py-3 text-center text-[10px] sm:text-xs font-bold text-slate-500 uppercase tracking-wider whitespace-nowrap">{{ __('Aksi') }}</th>
                    </tr>
                </thead>
                    ...
                    <tr class="hover:bg-slate-50/60 transition">
                        <td class="px-3 sm:px-4 py-3">
                            <div class="flex items-center space-x-2 sm:space-x-3">
                                <div class="w-7 h-7 sm:w-8 sm:h-8 rounded-lg bg-blue-50 flex items-center justify-center text-blue-600 border border-blue-100 flex-shrink-0">
                                    @php
                                ...
                                <div class="min-w-0">
                                    <span class="block font-bold text-slate-800 text-xs sm:text-sm leading-tight truncate max-w-[80px] sm:max-w-none">{{ $item->id_aset }}</span>
                                    <span class="block text-[9px] sm:text-[10px] text-slate-400 font-mono truncate max-w-[80px] sm:max-w-none">{{ $item->model }}</span>
                                </div>
                            </div>
                        </td>
                        ...
                        <td class="px-3 sm:px-4 py-3 text-xs sm:text-sm font-semibold text-slate-700 whitespace-nowrap hidden sm:table-cell">{{ number_format($item->total_operasi, 1) }}</td>
                        <td class="px-3 sm:px-4 py-3 text-xs sm:text-sm font-semibold text-slate-700 whitespace-nowrap hidden sm:table-cell">{{ number_format($item->total_idle, 1) }}</td>
                        <td class="px-3 sm:px-4 py-3">
                        ...
                        <td class="px-3 sm:px-4 py-3 text-xs sm:text-sm text-slate-700 whitespace-nowrap hidden md:table-cell">
                            {{ number_format($item->total_bakar ?? 0, 0) }}
                        </td>
                        <td class="px-3 sm:px-4 py-3 text-center">
                            <a href="{{ route('monitoring.detail', $item->id_aset) }}?bulan={{ $bulan }}&tahun={{ $tahun }}"
                               class="inline-flex items-center px-2.5 py-1.5 rounded-lg border border-slate-200 bg-slate-50 text-slate-600 hover:bg-slate-100 text-xs font-bold transition whitespace-nowrap">
                                <i class="fas fa-eye mr-1 text-slate-400"></i> {{ __('Detail') }}
                            </a>
                        </td>
                    </tr>
                    @empty
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    fetch('{{ route("monitoring.chart") }}?bulan={{ $bulan }}&tahun={{ $tahun }}')
        ...
        .catch(err => {
            ...
            const parent = document.getElementById('monitoringChart').parentElement;
            if (parent) {
                parent.innerHTML = `<div class="flex flex-col items-center justify-center h-full text-slate-400 py-10"><i class="fas fa-exclamation-triangle text-2xl text-amber-500 mb-2"></i><p class="text-xs font-semibold">Gagal memuat data grafik harian.</p></div>`;
            }
        });
