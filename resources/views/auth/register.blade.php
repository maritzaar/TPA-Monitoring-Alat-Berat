...
<body class="bg-gradient-to-br from-[#8E6E4F] via-[#A7825E] to-[#B49A80] min-h-screen flex items-center justify-center p-4 relative overflow-hidden">
    ...
    <div class="w-full max-w-md bg-white/10 backdrop-blur-xl border border-white/20 rounded-2xl shadow-2xl p-8 relative z-10">
        ...
        <div class="text-center mb-6">
            ...
            <h2 class="text-2xl font-bold text-white tracking-wide">{{ __('Buat Akun Baru') }}</h2>
            <p class="text-stone-200 text-sm mt-1">{{ __('Daftarkan diri Anda untuk masuk ke sistem') }}</p>
        </div>
        ...
        <form action="{{ route('register') }}" method="POST" class="space-y-4">
            ...
            <!-- Name Input -->
            <div>
                <label for="name" class="block text-xs font-semibold text-[#FAF7F2] uppercase tracking-wider mb-1.5">{{ __('Nama Lengkap') }}</label>
                <div class="relative">
                    <span class="absolute inset-y-0 left-0 pl-3.5 flex items-center text-stone-300">
                        <i class="far fa-user"></i>
                    </span>
                    <input type="text" name="name" id="name" required value="{{ old('name') }}"
                           class="block w-full pl-10 pr-4 py-2.5 bg-white/10 border border-white/20 rounded-xl text-white placeholder-stone-300 focus:outline-none focus:ring-2 focus:ring-white/50 focus:border-white transition text-sm"
            ...
            <!-- Email Input -->
            <div>
                <label for="email" class="block text-xs font-semibold text-[#FAF7F2] uppercase tracking-wider mb-1.5">{{ __('Alamat Email') }}</label>
                <div class="relative">
                    <span class="absolute inset-y-0 left-0 pl-3.5 flex items-center text-stone-300">
            ...
            <!-- Password Input -->
            <div>
                <label for="password" class="block text-xs font-semibold text-[#FAF7F2] uppercase tracking-wider mb-1.5">{{ __('Kata Sandi') }}</label>
                <div class="relative">
                    <span class="absolute inset-y-0 left-0 pl-3.5 flex items-center text-stone-300">
            ...
            <div>
                <label for="password_confirmation" class="block text-xs font-semibold text-[#FAF7F2] uppercase tracking-wider mb-1.5">{{ __('Konfirmasi Kata Sandi') }}</label>
                <div class="relative">
                    <span class="absolute inset-y-0 left-0 pl-3.5 flex items-center text-stone-300">
                        <i class="fas fa-lock"></i>
                    </span>
        ...
        <div class="mt-6 text-center text-sm">
            <span class="text-stone-200">{{ __('Sudah punya akun?') }}</span>
            <a href="{{ route('login') }}" class="text-[#FAF7F2] hover:text-white font-semibold ml-1 transition hover:underline">{{ __('Masuk di sini') }}</a>
        </div>
    </div>
</body>
</html>
